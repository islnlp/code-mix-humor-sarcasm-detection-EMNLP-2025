...
if task == 'humor' and MTL == 'humor,sarcasm':
    ...
    a1_eng_path = '/data1/debajyoti/code-mix-humor-sarcasm-detection/Dataset/JokesMix.16000/humor_English_16000.csv'  # auxilliary task eng dataset path
...
hum_com
com_1 = pd.concat([hate_com,hum_com])
...
class CustomDataset(Dataset):
    def __init__(self, sentences, labels1, labels2, tokenizer, max_len):
...
train_dataset = CustomDataset(data_tr['Sentence'].values, data_tr['Task1'].values, data_tr['Task2'].values, tokenizer, max_len)
val_dataset = CustomDataset(data_val['Sentence'].values, data_val['Task1'].values, data_val['Task2'].values, tokenizer, max_len)
test_dataset = CustomDataset(data_test['Sentence'].values, data_test['Task1'].values,data_test['Task2'].values, tokenizer, max_len)
...
def custom_joint_loss(logits_task1, logits_task2, labels_task1, labels_task2):
    ...
    # Combine the losses
    # joint_loss = 2*loss_task2 + loss_task1_2 + model.calc_regularization_loss()
...
for epoch in range(epochs):
    ...
    for batch in tqdm(train_dataloader):
        ...
        logits_task1, logits_task2 = model(input_ids, attention_mask=attention_mask)  # Get logits for both tasks

        loss = custom_joint_loss(logits_task1, logits_task2, labels_task1, labels_task2)  # Use custom joint loss
    ...
    for batch in val_dataloader:
        ...
        val_loss += custom_joint_loss(logits_task1, logits_task2, labels_task1, labels_task2).item()
